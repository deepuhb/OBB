...
def _cfg_to_params(cfg: Any, params: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Merge (defaults <- cfg.evaluator or cfg <- params) and coerce types."""
    def to_dict(x):
        if x is None:
            return {}
    ...
    d = dict(
        ...
        # legacy alias; if provided, overrides conf_thres
        score_thresh=None,

        # metrics
        map_iou_st=0.50, map_iou_ed=0.95, map_iou_step=0.05,
        pck_alpha=0.05,
    ...
    # type coercions
    for k in ("conf_thres", "iou_thres", "pck_alpha"):
        d[k] = float(d[k])
    for k in ("multi_label", "agnostic", "use_nms"):
        d[k] = bool(d[k])
    for k in ("max_det", "topk_kpt", "print_every"):
        d[k] = int(d[k])

    # legacy mapping
    if d["score_thresh"] is not None:
        d["conf_thres"] = float(d["score_thresh"])
    del d["score_thresh"]
...
def _get_first_batch(dl):
    ...
    if isinstance(batch, dict):
        imgs = batch.get("image") or batch.get("imgs") or batch.get("images") or batch.get("img")
...
class Evaluator:
    ...
    Assumes model implements:
      ...
      - decode_obb_from_pyramids(det_maps, imgs, conf_thres, iou_thres, multi_label, agnostic, max_det, use_nms)
        -> list[{'boxes','obb','scores','labels'}]
      - (optional) kpt_from_obbs(feats, obb_list, scores_list=None, topk, chunk, score_thresh)
          returns (uv_all, metas) where uv_all in [0,1], metas[i]['M'] is 2x3 crop->feature affine,
          metas[i]['bix'] image index, and metas[i]['oi'] per-image box index.
    """
    ...
    def evaluate(self,
        ...
        for batch in val_loader:
            ...
            if isinstance(outs, dict):
                det_maps = outs["det"]
                feats = outs.get("feats")
            elif isinstance(outs, (list, tuple)):
                det_maps = outs[0]
                feats = outs[1] if len(outs) > 1 else None
            else:
                det_maps, feats = outs, None

            # ---- decode via the model's canonical decoder (YOLO11-style) ----
            try:
                ...
                # if the model still has legacy kwargs, try legacy mapping
                preds_list = model_eval.decode_obb_from_pyramids(
                    det_maps, imgs,
            ...
            gtl_list = batch.get("labels", [None for _ in range(B)])  # list[Ti] or None
            ...
            if hasattr(model_eval, "kpt_from_obbs") and feats is not None:
                try:
                    ...
                    uv_all, metas = model_eval.kpt_from_obbs(
                        feats, obb_for_kpt,
                        scores_list=scores_for_kpt,
                        topk=min(self.cfg["topk_kpt"], self.cfg["max_det"]),
                        chunk=128,
                        score_thresh=self.cfg["conf_thres"]
                    )
                    if uv_all is not None and len(metas) == uv_all.shape[0]:
                        # map (u,v)∈[0,1] → image px: meta['M'] maps to feature coords; multiply by feat_down
                        ...
                        for (uv, meta) in zip(uv_all, metas):
                            ...
                            xy_feat = (M @ xy1)  # (2,) in feature pixels (/8)
            ...
            for b in range(B):
                ...
                if gtl_list[b] is not None:
                    gt_labels = torch.as_tensor(gtl_list[b], device=device, dtype=torch.long)
                ...
                # For per-class matching, keep a "used" mask per GT index
                ...
                for t in self.iou_thrs:
                    ...
                    for i in order.tolist():
                        ...
                        # class-aware matching if labels & gt_labels are provided
                        if labels is not None and gt_labels is not None and gt_labels.numel():
                            # candidate GTs with same class
                            same_class = (gt_labels == labels[i])
                            if not same_class.any():
                                tp.append(0); fp.append(1); continue
                            # among same-class, pick best IoU
                            cand_idx = same_class.nonzero(as_tuple=True)[0]
                            if iou_mat.numel():
                                ious = iou_mat[i, cand_idx]
                                j_rel = int(torch.argmax(ious))
                                j = int(cand_idx[j_rel].item())
                                iou_ij = float(ious[j_rel].item())
                            else:
                                j, iou_ij = 0, 0.0
                        else:
                            # class-agnostic
    ...
    def _iou_matrix(self, boxes: torch.Tensor, gts: torch.Tensor) -> torch.Tensor:
        ...
        if _MMCV_OK:
            ...
            b = boxes.detach().clone(); g = gts.detach().clone()
