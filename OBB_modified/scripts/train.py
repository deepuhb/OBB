...
def build_loaders(args: argparse.Namespace, distributed: bool):
    ...
    if distributed:
        train_sampler = DistributedSampler(train_ds, num_replicas=args.world_size, rank=args.rank, shuffle=True,
                                           drop_last=False)
        val_sampler = DistributedSampler(val_ds, num_replicas=args.world_size, rank=args.rank, shuffle=False,
                                         drop_last=False)
    else:
        train_sampler = None
        val_sampler = None
...
def main() -> None:
    ...
    # Mixed precision scaler – use the new torch.amp API instead of the deprecated torch.cuda.amp
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            optimizer.zero_grad()
            # Forward + loss under AMP context using torch.amp.autocast
