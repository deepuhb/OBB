...
def _cfg_to_params(cfg: Any, params: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    del d["score_thresh"]
    d["map_iou_st"] = float(d["map_iou_st"]); d["map_iou_ed"] = float(d["map_iou_ed"]); d["map_iou_step"] = float(d["map_iou_step"])
    return d
...
class Evaluator:
    def __init__(self, cfg: Optional[Any] = None, names: Optional[List[str]] = None,
        ...
        self.iou_thrs = np.arange(self.cfg["map_iou_st"], self.cfg["map_iou_ed"] + 1e-9, self.cfg["map_iou_step"]).round(2)
    ...
    def evaluate(self, model: torch.nn.Module, val_loader, device: torch.device, max_images: Optional[int] = None) -> Dict[str, float]:
        ...
        for batch in val_loader:
            ...
            if not isinstance(batch, dict) or ("image" not in batch):
                self.log.warning("[eval] Unexpected batch format; skipping."); continue
            ...
            if hasattr(model_eval, "kpt_from_obbs") and feats is not None:
                try:
                    obb_for_kpt    = [pred["boxes"].clone() if ("boxes" in pred) else pred.get("obb", torch.zeros((0,5), device=device)) for pred in preds_list]
                    ...
                    if uv_all is not None and len(metas) == uv_all.shape[0]:
                        ...
                        for (uv, meta) in zip(uv_all, metas):
                            ...
                            M = meta["M"]; bix = int(meta["bix"]) if isinstance(meta["bix"], int) else int(meta["bix"].item())
                            if not torch.is_tensor(M): M = torch.as_tensor(M, dtype=torch.float32, device=device)
                            M = M.to(device).float()
                            x = uv[0] * 2.0 - 1.0; y = uv[1] * 2.0 - 1.0
                            xy1 = torch.stack([x, y, torch.tensor(1.0, device=device)], dim=0)
                            xy_feat = (M @ xy1); xy_img = xy_feat * float(feat_down)
            ...
            for b in range(B):
                try:
                    img_seen += 1; stats["images_eval"] += 1
                    gt_boxes = gtb_list[b].to(device); gt_kpts = gtk_list[b].to(device)
                    gt_labels = None
                    ...
                    stats["pred_count"] += int(boxes.shape[0]); stats["gt_total"] += int(gt_boxes.shape[0])
                    ...
                    if uv_img.numel() and gt_kpts.numel():
                        ...
                        n = uv_img.shape[0]
                        thrv = (self.cfg["pck_alpha"] * (sizes[:n] if sizes.numel() >= n else sizes[-1].repeat(n))) if sizes.numel() else torch.full((n,), 1.0, device=device)
    ...
    def _iou_matrix(self, boxes: torch.Tensor, gts: torch.Tensor) -> torch.Tensor:
        ...
        if _MMCV_OK:
            ...
            g = gts.detach().clone()
            # mmcv uses degrees
            b[:, 4] = torch.rad2deg(b[:, 4]); g[:, 4] = torch.rad2deg(g[:, 4])
            try:
    ...
    def _iou_shapely(self, a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
        ...
        ap = self._to_poly(an); bp = self._to_poly(bn)
    ...
    def _iou_aabb_pair(self, a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
        ...
        a1, a2, a3, a4 = aabb(a); b1, b2, b3, b4 = aabb(b)
        A, B = a.shape[0], b.shape[0]
        out = torch.zeros((A, B), dtype=torch.float32)
        for i in range(A):
            for j in range(B):
                xx1 = max(float(a1[i]), float(b1[j])); yy1 = max(float(a2[i]), float(b2[j]))
                xx2 = min(float(a3[i]), float(b3[j])); yy2 = min(float(a4[i]), float(b4[j]))
                iw = max(0.0, xx2 - xx1); ih = max(0.0, yy2 - yy1)
                inter = iw * ih
                ua = (float(a3[i] - a1[i]) * float(a4[i] - a2[i])); ub = (float(b3[j] - b1[j]) * float(b4[j] - b2[j]))
                union = ua + ub - inter
    ...
    def _ap_from_pr(self, tp: np.ndarray, fp: np.ndarray, sc: np.ndarray) -> float:
        ...
        order = np.argsort(-sc); tp = tp[order].astype(np.float32); fp = fp[order].astype(np.float32)
        ctp = np.cumsum(tp); cfp = np.cumsum(fp)
        denom = max(1.0, float(tp.sum()))
        recall = ctp / denom; precision = ctp / np.maximum(ctp + cfp, 1e-9)
        rec_points = np.linspace(0, 1, 101); prec_at_rec = np.zeros_like(rec_points)
    ...
    def _finalize_and_log(self, st: dict) -> dict:
        ...
        try:
            i50 = list(self.iou_thrs).index(0.5); mAP50 = float(aps[i50]) if aps else 0.0
        except ValueError:
            mAP50 = 0.0
...
(self, obb_np: np.ndarray) -> List["Polygon"]:
        out = []
        for cx, cy, w, h, ang in obb_np:
            ca, sa = math.cos(float(ang)), math.sin(float(ang))
            dx, dy = w / 2.0, h / 2.0
    ...
    def _iou_aabb_pair(self, a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
        ...
        def aabb(x):
            ...
            return cx - w/2, cy - h/2, cx + w/2, cy + h/2
        a1, a2, a3, a4 = aabb(a); b1, b2, b3, b4 = aabb(b)
        A, B = a.shape[0], b.shape[0]; out = torch.zeros((A, B))
        for i in range(A):
            for j in range(B):
                xx1 = max(float(a1[i]), float(b1[j])); yy1 = max(float(a2[i]), float(b2[j]))
                xx2 = min(float(a3[i]), float(b3[j])); yy2 = min(float(a4[i]), float(b4[j]))
