...
def _cfg_to_params(cfg: Any, params: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    # Defaults mirror your working file, plus 'per_image_debug'
    d = dict(conf_thres=0.25, iou_thres=0.70, multi_label=False, agnostic=False, use_nms=True, max_det=300,
             ...
             topk_kpt=128, print_every=0, per_image_debug=3)
    ...
    for k in ("max_det", "topk_kpt", "print_every", "per_image_debug"): d[k] = int(d[k])
    # env override for per-image summaries if provided
    env_k = os.getenv("EVAL_PER_IMAGE_DEBUG", "")
    if env_k.isdigit(): d["per_image_debug"] = int(env_k)
...
class Evaluator:
    ...
    def evaluate(self, model: torch.nn.Module, val_loader, device: torch.device, max_images: Optional[int] = None) -> Dict[str, float]:
        ...
        for batch in val_loader:
            ...
            # ---- One-time global debug (pred stats + GT stats + IoU probe) ----
            if not self._once_global_debug:
                self._once_global_debug = True
                try:
                    sc = np.concatenate([p["scores"].detach().float().cpu().numpy()
                                         for p in preds_list if p["scores"].numel()]) if preds_list else np.array([])
                    ws = np.concatenate([p["boxes"].detach().float().cpu().numpy()[:, 2]
                                         for p in preds_list if p["boxes"].numel()]) if preds_list else np.array([])
                    hs = np.concatenate([p["boxes"].detach().float().cpu().numpy()[:, 3]
                                         for p in preds_list if p["boxes"].numel()]) if preds_list else np.array([])
                    if sc.size:
                        q = np.quantile(sc, [0.5, 0.9, 0.99])
                        print(f"[EVAL DEBUG] preds/batch={sc.size}  score med={q[0]:.3f} p90={q[1]:.3f} p99={q[2]:.3f}")
                    else:
                        print("[EVAL DEBUG] preds/batch=0")
                    if ws.size:
                        q_w = np.quantile(ws, [0.5, 0.9, 0.99]); q_h = np.quantile(hs, [0.5, 0.9, 0.99])
                        print(f"[EVAL DEBUG] box_w med/p90/p99 = {q_w[0]:.1f}/{q_w[1]:.1f}/{q_w[2]:.1f}  "
                              f"box_h med/p90/p99 = {q_h[0]:.1f}/{q_h[1]:.1f}/{q_h[2]:.1f}")
                    # GT stats
                    gt_boxes_np = np.concatenate([g.detach().float().cpu().numpy()
                                                  for g in gtb_list if (g is not None and g.numel())], axis=0) \
                                  if isinstance(gtb_list, list) and len(gtb_list) else np.zeros((0,5), dtype=np.float32)
                    if gt_boxes_np.size:
                        gw = gt_boxes_np[:, 2]; gh = gt_boxes_np[:, 3]; ga = gt_boxes_np[:, 4]
                        gqw = np.quantile(gw, [0.5, 0.9, 0.99]); gqh = np.quantile(gh, [0.5, 0.9, 0.99])
                        print(f"[EVAL DEBUG GT] gt/batch={gt_boxes_np.shape[0]}  "
                              f"w med/p90/p99 = {gqw[0]:.1f}/{gqw[1]:.1f}/{gqw[2]:.1f}  "
                              f"h med/p90/p99 = {gqh[0]:.1f}/{gqh[1]:.1f}/{gqh[2]:.1f}")
                        longer = np.maximum(gw, gh); shorter = np.minimum(gw, gh)
                        ar = longer / np.maximum(1e-6, shorter)
                        qar = np.quantile(ar, [0.5, 0.9, 0.99])
                        print(f"[EVAL DEBUG GT] aspect longer/shorter med/p90/p99 = {qar[0]:.2f}/{qar[1]:.2f}/{qar[2]:.2f}")
                        gmean = float(np.mean(ga)); gstd = float(np.std(ga)); gmin = float(np.min(ga)); gmax = float(np.max(ga))
                        print(f"[EVAL DEBUG GT] angle rad mean/std = {gmean:.3f}/{gstd:.3f}  min/max = {gmin:.3f}/{gmax:.3f}")
                    else:
                        print("[EVAL DEBUG GT] gt/batch=0")
                    # IoU probe for img0 if both exist
                    if (isinstance(gtb_list, list) and len(gtb_list) and gtb_list[0] is not None and
                        preds_list and preds_list[0]["boxes"].numel()):
                        A = preds_list[0]["boxes"].detach()
                        G = gtb_list[0].to(A.device) if hasattr(gtb_list[0], "to") else gtb_list[0]
                        ious = self._iou_matrix(A, G)
                        if ious.numel():
                            mx = ious.max(dim=0).values.cpu().numpy()
                            qs = np.quantile(mx, [0.5, 0.9, 0.99, 1.0])
                            print(f"[EVAL DEBUG] best IoU per-GT (img0): med={qs[0]:.3f} p90={qs[1]:.3f} p99={qs[2]:.3f} max={qs[3]:.3f}")
                except Exception as _e:
                    print(f"[EVAL DEBUG] stats error: {_e}")

            # ---- Per-image summaries (first K images only) ----
            K = int(self.cfg.get("per_image_debug", 3))
            for b in range(B):
                if self._printed_images < K:
                    self._per_image_summary(b, preds_list[b], gtb_list[b], iou_thr=float(self.cfg["iou_thres"]))
                    self._printed_images += 1

                # ---- Accumulate metrics for mAP ----
                try:
                    stats["images_eval"] += 1
                    ...
                    msg = (
                        ...
                        f"  gt_boxes: {_shape(gt_boxes)}  gt_labels: {_shape(gt_labels)}  gt_kpts: {_shape(gtk_list[b])}\n"
    ...
    # --------- tiny helper for per-image summaries ---------
    def _per_image_summary(self, idx: int, pred: Dict[str, torch.Tensor], gt: torch.Tensor, iou_thr: float = 0.5):
        boxes = pred.get("boxes", torch.zeros((0, 5)))
        scores = pred.get("scores", torch.zeros((0,)))
        labels = pred.get("labels", torch.zeros((0,), dtype=torch.long))
        print(f"[EVAL IMG {idx}] pred={int(boxes.shape[0])} gt={int(gt.shape[0])}")

        if boxes.numel():
            b = boxes.detach().float().cpu().numpy()
            w, h = b[:, 2], b[:, 3]
            s = scores.detach().float().cpu().numpy() if scores.numel() else np.zeros((b.shape[0],), dtype=np.float32)
            q_w = np.quantile(w, [0.5, 0.9]); q_h = np.quantile(h, [0.5, 0.9])
            print(f"  pred w med/p90 = {q_w[0]:.1f}/{q_w[1]:.1f}  h med/p90 = {q_h[0]:.1f}/{q_h[1]:.1f}  "
                  f"score mean/max = {s.mean():.3f}/{(s.max() if s.size else 0):.3f}")
        else:
            print("  pred empty")

        if gt is not None and gt.numel():
            g = gt.detach().float().cpu().numpy()
            gw, gh = g[:, 2], g[:, 3]
            q_w = np.quantile(gw, [0.5, 0.9]); q_h = np.quantile(gh, [0.5, 0.9])
            print(f"  GT   w med/p90 = {q_w[0]:.1f}/{q_w[1]:.1f}  h med/p90 = {q_h[0]:.1f}/{q_h[1]:.1f}")
        else:
            print("  GT empty")

        if boxes.numel() and gt is not None and gt.numel():
            ious = self._iou_matrix(boxes, gt)
            best_per_gt = ious.max(dim=0).values if ious.numel() else torch.zeros((gt.shape[0],), device=boxes.device)
            best_per_pr = ious.max(dim=1).values if ious.numel() else torch.zeros((boxes.shape[0],), device=boxes.device)
            r = float((best_per_gt >= iou_thr).float().mean().item()) if best_per_gt.numel() else 0.0
            print(f"  IoU: recall@{iou_thr:.2f}={r:.3f}  bestIoU(gt) med/max = "
                  f"{best_per_gt.median().item() if best_per_gt.numel() else 0:.3f}/"
                  f"{best_per_gt.max().item() if best_per_gt.numel() else 0:.3f}  "
                  f"bestIoU(pred) med/max = "
                  f"{best_per_pr.median().item() if best_per_pr.numel() else 0:.3f}/"
                  f"{best_per_pr.max().item() if best_per_pr.numel() else 0:.3f}")
        else:
            print("  IoU: (not computed)")
