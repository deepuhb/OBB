...
class OBBKeypointLoss:
    ...
    def __call__(self, preds, batch):
        ...
        pred_scores = pred_scores.permute(0, 2, 1).contiguous()
        pred_kpts = pred_kpts.permute(0, 2, 1).contiguous()
        pred_dist_obb = pred_dist_obb.permute(0, 2, 1).contiguous()
        dtype = pred_scores.dtype
        ...
        pred_bboxes = self.decode_obb(anchor_points, pred_dist_obb)
        _, target_bboxes, target_scores, fg_mask, target_gt_idx = self.assigner(
            pred_scores.detach().sigmoid(),
    ...
    def decode_obb(self, anchor_points, pred_dist_obb):
        pred_dist, pred_rot = pred_dist_obb.split((self.reg_max * 4, 1), 2)
        box_xywh = dist2bbox(self.dfl(pred_dist), anchor_points, xywh=True, dim=2)
