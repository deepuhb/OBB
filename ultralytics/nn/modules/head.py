...
class OBBKeypointDetect(Detect):
    """
    An independent, unified detection head for simultaneous OBB and XY-Keypoint detection.
    ...
    def __init__(self, nc=80, kpt_shape=(1, 2), ch=()):
        ...
        self.ch = ch  # Store input channels
        ...
        # This follows the decoupled head design pattern of the official OBB and Pose heads.
    ...
    def forward(self, x):
        """
        The forward pass now processes inputs through each decoupled branch
        and concatenates the results into a single unified tensor.
        """
        shape = x.shape  # for autoshaping
        # Process each feature map from the neck
        for i in range(self.nl):
            # Apply each specialized branch to the input feature map
            # self.cv2 and self.cv3 are inherited from the parent 'Detect' class
            x_box = self.cv2[i](x[i])
            x_cls = self.cv3[i](x[i])
            x_angle = self.cv4[i](x[i])
            x_kpt = self.cv5[i](x[i])
            # Concatenate the outputs along the channel dimension in a consistent order
            x[i] = torch.cat((x_box, x_angle, x_cls, x_kpt), 1)
        ...
        # Decode Keypoints
        kpts = self.decode_kpts(kpts.permute(0, 2, 1).contiguous(), anchor_points, stride_tensor)
        obb = xywhr2xyxyxyxy(box_xywhr)

        y = torch.cat((obb, cls.permute(0, 2, 1).contiguous().sigmoid(), kpts), 2)
        return y if self.export else (y, x)

    def decode_kpts(self, kpts, anchors, stride):
        """Decodes keypoint predictions for XY coordinates only."""
        y = kpts.clone()
        y = y.view(y.shape, y.shape[1], self.nk, self.kp_param)
