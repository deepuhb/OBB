...
class TDOBBWKpt1Criterion(nn.Module):
    ...
    def _iou_rot(self, a, b):
        ...
        try:
            ...
            for i in range(A.shape[0]):
                ...
                xx1 = torch.maximum(ax1, bx1); yy1 = torch.maximum(ay1, by1)
                xx2 = torch.minimum(ax2, bx2); yy2 = torch.minimum(ay2, by2)
                iw = (xx2-xx1).clamp_min(0); ih = (yy2-yy1).clamp_min(0)
                inter = iw*ih
    ...
    def _loss_det(
        ...
        obj_pos_terms = 0
        obj_neg_terms = 0
        neg_per_pos = 64  # hard-negative samples per positive

        # debug buckets (first few steps)
        dbg_pred_w, dbg_pred_h, dbg_gt_w, dbg_gt_h = {0: [], 1: [], 2: []}, {0: [], 1: [], 2: []}, {0: [], 1: [], 2: []}, {0: [], 1: [], 2: []}
        ...
        for b in range(B):
            ...
            for n in range(boxes.shape[0]):
                cx, cy, w, h, ang_gt = [float(x) for x in boxes[n]]
                max_side = max(w, h)
                ...
                # ------------ center sampling: 3x3 around (i0,j0) ------------
                r = 1  # fixed 3x3 neighborhood
                for dj in range(-r, r + 1):
                    for di in range(-r, r + 1):
                        ...
                        # ---------------- objectness with IoU target ----------------
                        pos_logit = mp["obj"][b, j, i]
                        with torch.no_grad():
                            cx_hat, cy_hat, pw_hat, ph_hat, ang_hat = self._decode_at_cell(mp, b, j, i, stride)
                            pred = torch.tensor([[cx_hat, cy_hat, pw_hat, ph_hat, ang_hat]], device=device, dtype=mp["tw"].dtype)
                            gt   = torch.tensor([[cx,     cy,     w,      h,      ang_gt]], device=device, dtype=mp["tw"].dtype)
                            iou_obj = self._iou_rot(pred, gt).squeeze().clamp(0, 1)
                        l_obj = l_obj + F.binary_cross_entropy_with_logits(pos_logit, iou_obj)
                        obj_pos_terms += 1
                        ...
                        sin_t = torch.tensor(math.sin(ang_gt), device=device, dtype=sin_p.dtype)
                        cos_t = torch.tensor(math.cos(ang_gt), device=device, dtype=cos_p.dtype)
                        ...
                        with torch.no_grad():
                            _, _, pw_hat_dbg, ph_hat_dbg, _ = self._decode_at_cell(mp, b, j, i, stride)
                            dbg_pred_w[lvl].append(float(pw_hat_dbg))
                            dbg_pred_h[lvl].append(float(ph_hat_dbg))
        ...
        if (total_pos > 0) and (not hasattr(self, "_DBG_DET_ONCE") or not self._DBG_DET_ONCE):
            for lvl, name in enumerate(("P3", "P4", "P5")):
                if dbg_pred_w[lvl]:
                    pw = np.median(dbg_pred_w[lvl]); ph = np.median(dbg_pred_h[lvl])
                    gw = np.median(dbg_gt_w[lvl]);   gh = np.median(dbg_gt_h[lvl])
    ...
    def _loss_kpt(
        ...
        for b in range(B):
            ...
            for j, md in enumerate(metas_b):
                ...
                uv_j = _img_kpts_to_crop_uv(xy_img, M_crop_to_feat, feat_down=feat_down)  # (1,2) in crop px
                uv_gt_list.append(uv_j)

            uv_b_gt = torch.cat(uv_gt_list, dim=0) if uv_gt_list else torch.zeros((0, 2), device=device, dtype=uv_b_pred.dtype)
